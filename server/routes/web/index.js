module.exports = app => {
    ...
    //录入新闻数据的测试接口  意思是：这里是写的代码可以自动导入数据库 也可以在后台手动导入  
    router.get('/news/init', async (req, res) => {
        ...
        const cats = await Category.find().where({
            parent: parent
        }).lean()   //lean 表示取纯粹的json(JS对象和数组) 、不带任何mongoose模型对象
        ...
        const newsList = newsTitles.map(title => {
            //slice(0)截取全部内容返回新数组   新数组随机排序
          const randomCats = cats.slice(0).sort((a, b) => Math.random() - 0.5)
          return {
            //categories选随机排序之后的前两个
            categories: randomCats.slice(0, 2),
            title: title
          }
        })
        await Article.deleteMany({})    //原有的数据库清空   {}代表以任意条件去查询
    ...
    router.get('/news/list', async (req, res) => {  
        ...
        //聚合查询 同时执行好几次查询  通过match过滤一些数据  然后关联查询  然后修改newsList
        const cats = await Category.aggregate([
            //$match 条件查询   类似where操作
            { $match: { parent: parent._id } },
            //lookup 类似关系数据库的join 做外连接
            {
                $lookup: {
                from: 'articles',   //关联的集合  文章
                localField: '_id',
            ...
            {
                $addFields: {
                //$slice操作符 第一个参数是字段   第二个参数是数量
                newsList: { $slice: ['$newsList', 5] }
                }
            }
        ])
        const subCats = cats.map(v => v._id)
        //在cats数组前加一个热门分类  从新闻、公告、活动、赛事获取5个
        cats.unshift({
            name: '热门',
        ...
        //热门、新闻、公告、活动、赛事的内容开头名称  热门则显示所属分类名称
        cats.map(cat => {
